...
// The PWM-frequency field: a type Choice plus a custom-value NumberEdit shown
// only for the "custom" selection. Flowed side-by-side into a ds::FieldGroup's
// content area (which owns the layout, gaps and touch floor), so this helper
// just builds the two controls into that content rather than being a Window
// itself. The NumberEdit is created before the Choice's set handler is wired,
// so the (persistent) edit pointer can be captured by value into the handler
// while the controls still flow in the original Choice-then-edit order.
struct PWMfrequencyChoice {
  PWMfrequencyChoice(Window* content, uint8_t moduleIdx, uint8_t channelIdx);
  PWMfrequencyChoice(Window* content, uint8_t moduleIdx);
};

PWMfrequencyChoice::PWMfrequencyChoice(Window* content, uint8_t moduleIdx, uint8_t channelIdx)
{
  ...
  auto choice = new Choice(content, rect_t{}, _v1_pwmfreq_types, 0, 4,
                [=,&pwmvalue_type]{
                      return pwmvalue_type;
                  });
  auto num_edit = new NumberEdit(content, {0, 0, EdgeTxStyles::EDIT_FLD_WIDTH_NARROW, 0}, 50, 400,
  ...
  choice->setSetValueHandler([=,&pwmvalue_type](int32_t newValue) {
...
PWMfrequencyChoice::PWMfrequencyChoice(Window* content, uint8_t moduleIdx )
{
  ...
  auto choice = new Choice(content, rect_t{}, _v0_pwmfreq_types, 0, 2,
                [=,&pwmvalue_type]{
                      return pwmvalue_type;
                });
  auto num_edit = new NumberEdit(content, {0, 0, EdgeTxStyles::EDIT_FLD_WIDTH_NARROW, 0}, 50, 400,
...
AFHDS3_Options::AFHDS3_Options(uint8_t moduleIdx, Route route) : Page(ICON_MODEL_SETUP, route)
{
  ...
  if (cfg->version == 0) {
    ...
    new ds::FieldGroup(body, temp_str.c_str(), [=](Window* content) {
      PWMfrequencyChoice(content, moduleIdx);
    });

    temp_str = "PWM ";
    temp_str += STR_SYNC;
    new ds::FormRow(body, temp_str.c_str(), [=](Window* slot) {
      new ToggleSwitch(slot, rect_t{}, GET_SET_AND_SYNC(cfg, vCfg->PWMFrequency.Synchronized,
    ...
    temp_str += " 1";
    new ds::FormRow(body, temp_str.c_str(), [=](Window* slot) {
      new Choice(slot, rect_t{}, _analog_outputs,
    ...
    new ds::FormRow(body, STR_SERIAL_BUS, [=](Window* slot) {
      new Choice(slot, rect_t{}, _bus_types, 0, 2,
    ...
    for (uint8_t i = 0; i < channel_num[vCfg->PhyMode]; i++) {
      ...
      new ds::FieldGroup(body, temp_str.c_str(), [=](Window* content) {
        PWMfrequencyChoice(content, moduleIdx, i);
      });
      ...
      new ds::FormRow(body, temp_str.c_str(), [=](Window* slot) {
        new ToggleSwitch(
            slot, rect_t{}, GET_DEFAULT((vCfg->PWMFrequenciesV1.Synchronized&1<<i)>>i),
            [=](uint8_t newVal) {
    ...
    for (uint8_t i = 0; i < SES_NPT_NB_MAX_PORTS; i++) {
      ...
      portName += 'A' + i;
      new ds::FormRow(body, portName.c_str(), [=](Window* slot) {
        new Choice(slot, rect_t{}, _v1_bus_types, afhds3::SES_NPT_PWM,
  ...
  new ds::FormRow(body, STR_SIGNAL_OUTPUT, [=](Window* slot) {
    new Choice(slot, rect_t{}, signed_strength_ch,
