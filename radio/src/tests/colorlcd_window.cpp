...
TEST(ColorWindow, TableFieldRowPressAcksBeforeHandler)
{
  const pid_t pid = fork();
  ASSERT_GE(pid, 0);
  ...
  if (pid == 0) {
    alarm(2);
    _exit(tableFieldRowPressAcksBeforeHandlerForTest() ? 0 : 1);
  }
...
TEST(ColorWindow, NumberEditDeleteWhileWheelOpenClosesWheel)
{
  const pid_t pid = fork();
  ASSERT_GE(pid, 0);
  ...
  if (pid == 0) {
    alarm(2);
    _exit(numberEditDeleteWhileWheelOpenClosesWheelForTest() ? 0 : 1);
  }
